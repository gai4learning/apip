                ...
                with st.spinner("Getting response..."):
                    ...
                    if model_type == "chat":
                        ...
                        if isinstance(custom_prompt, list):
                            ...
                            with st.container():
                                st.subheader("Embeddings and Similarities")
                                ...
                                st.write("Similarity Matrix (cosine similarity):")
                                df = pd.DataFrame(
                                    similarity_matrix,
                                    index=[f"Text {i+1}" for i in range(len(embeddings))],
                                    columns=[f"Text {i+1}" for i in range(len(embeddings))]
                                )
                                st.dataframe(df)
                                ...
                                if len(embeddings) > 1:
                                    ...
                                    st.info(f"""
                                    Most similar pair: Text {max_pair[0]+1} and Text {max_pair[1]+1}
                                    Similarity score: {max_sim:.4f}
                                    """)
